...
class RelatorioController extends Zend_Controller_Action
{
    ...
    public function relatorioDiarioClasseNotasAction()
    {      
       // action body
        $this->_helper->layout->disableLayout();
        $this->_helper->viewRenderer->setNoRender(true);
        ...
        $excel = new Aplicacao_Relatorio_Excel();
        $resultado = $excel->getRelatorioDiarioClasseNotas($mapper_turma->getAlunosOrganizadosByTurma(unserialize(base64_decode($this->getParam('turmas'))), false, (int) base64_decode($this->getParam('periodo')), true), base64_decode($this->getParam('formato')), $mapper_calendario->getDatasByPeriodo(new Application_Model_Periodo((int) base64_decode($this->getParam('periodo')))));
    ...
    public function opcoesRelatorioDiarioClasseNotasAction()
    {
        $usuario = Zend_Auth::getInstance()->getIdentity();

        $this->view->title = "Projeto Incluir - Relatório Diário de Classe - NOTAS";
        $form_opcoes_relatorio = new Application_Form_RelatorioFrequenciaAlunos();
        ...
        if ($this->_request->isPost()) {
            ...
            if ($form_opcoes_relatorio->isValid($dados))
                $this->_helper->redirector->goToRoute(array('controller' => 'relatorio', 'action' => 'relatorio-diario-classe-notas', 'turmas' => base64_encode(serialize($form_opcoes_relatorio->getValue('turmas'))), 'formato' => $form_opcoes_relatorio->getValue('formato_saida'), 'periodo' => $form_opcoes_relatorio->getValue('periodo')), null, true);
        }
        $this->view->form = $form_opcoes_relatorio;  
    }
